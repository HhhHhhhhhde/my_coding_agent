...
本项目已完成 Milestone 1-3 的主体能力，并补充了 Milestone 4 的轨迹回放首版和轻量 skill 机制：自研 agent loop、JSON action 协议、本地文件/搜索/命令工具、轨迹日志、Markdown 报告、Plan / Build 双模式、会话上下文、上下文压缩、持久 skill 指令、错误恢复提示、progress guard、workspace 权限边界、敏感文件保护和 shell 风险控制。CLI 还提供了一个零依赖文字交互界面，用于连续输入任务、切换模式、启停 skill、列出历史轨迹并回放某次运行。
...
- 轻量 Skill：已完成首版。可从 `skills/<name>/SKILL.md` 启用持久指令，也可让 agent 创建新 skill 文件。
...
Skill 文件默认放在当前 workspace 的 `skills/<name>/SKILL.md`。启用后的 skill 会跨多轮交互保持生效，直到执行 `/skill remove NAME` 或 `/skill clear`。用户可以手写 skill 文件，也可以直接让 agent 创建，例如：

```text
请新增一个 code-review skill，要求先列风险、再给修改建议，写到 skills/code-review/SKILL.md。
```

内置示例 skill：

- `skills/python-testing/SKILL.md`：偏向先读测试、最小修改、运行最小相关 pytest。
- `skills/safety-review/SKILL.md`：偏向先识别风险、权限边界和验证点。
- `skills/compact-planner/SKILL.md`：偏向输出短计划并快速收敛。
- `skills/demo-skill/SKILL.md`：数学函数演示用，要求 `sin`/`cos` 用轻量多项式近似，便于确认 skill 确实注入。
...
当前离线测试覆盖 agent loop、parser、工具协议、上下文管理、skill 注入、progress guard、target scope、session、CLI 展示、milestone03 安全策略和轨迹回放。

根目录下还提供一个本地演示流程文件 `DEMO_TEST_FLOW.md`，用于在 2 分钟内快速覆盖核心功能。该文件和 `README.txt` 一样被 `.gitignore` 忽略，不作为源码提交内容的一部分。
...
- `docs/specs/08-skills.md`：轻量 skill 机制、交互命令和持久化策略。

## 计划

当前已完成 Milestone 1-3 的主体实现，并完成轨迹回放与轻量 skill 首版。交付收尾材料已补齐：`README.txt` 精简版和 `DEMO_TEST_FLOW.md` 本地测试流程均放在根目录且不被 git 跟踪。Run index、轻量历史检索和更严格验证门禁作为后续加分项。
...
- `docs/plans/milestone-5-skills.md`
